...
type ContentListProps = {
  ...
  fallbackItemImage: Content.BlogIndexSlice["primary"]["fallback"];
...
export default function ContentList({
  ...
  useEffect(() => {
    // Animate how list-items show  with a stagger
  ...
  // on hover list item you show image
  const onMouseEnter = (index: number) => {
    setCurrentItem(index);
    if (!hovering) setHovering(true);
  };

  // remove img by setting currentItem to null on Ul
  const onMouseLeave = () => {
    setHovering(false);
    setCurrentItem(null);
  };

  // array of images from list
  const contentImages = items.map((item) => {
    const image = isFilled.image(item.data.hover_image)
      ? item.data.hover_image
      : fallbackItemImage;

    // helper func from prismic
