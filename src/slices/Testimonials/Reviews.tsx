...
const QuoteIcon = ({ className = "" }: { className?: string }) => (
  <div className={`text-4xl font-bold mb-3 ${className}`}>
    "
  </div>
);
...
const TestimonialCard = ({ item, index }: { item: any; index: number }) => {
  ...
  const cardClasses = `
    relative p-6 rounded-t-xl min-h-[200px] flex flex-col justify-between
  ...
  const headerIcon = headerType === "stars" ? <StarRating /> : <QuoteIcon className={backgroundType === "white" ? "text-black" : "text-white"} />;
  ...
  return (
    ...
    >
      {hasBackgroundImage && (
        <div className="absolute inset-0 bg-gradient-to-br from-blue-600/80 to-purple-600/80 rounded-t-xl"></div>
      )}
